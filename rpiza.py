...
# tkinter ist nur für die GUI erforderlich. In headless-Umgebungen (z. B. CI,
# Testrechner ohne Display) ist es evtl. nicht installiert. Die Steuerungslogik
# (RPiZA / FahrzeugSensor) läuft komplett ohne GUI, daher wird der Import
# optional gehalten. Die eigentliche Anlagenlogik bleibt unverändert.
try:
...
try:
    ...
    log.warning("RPi.GPIO nicht verfügbar — Mock-Modus aktiv.")
...
try:
    ...
    _MOCK = True
    log.warning("gpiozero nicht gefunden — Software-Stub aktiv.")
...
SENSOR_BEREICH_CM = 5
SENSOR_BEST = 3
# Abtastintervall des Fahrzeugsensors.
# 0,05 s hält die Erkennungslatenz (SENSOR_BEST × SENSOR_INTERVALL) unter der
# NFA-P01-Grenze von 200 ms, ohne die 3-fach-Entprellung aufzugeben.
SENSOR_INTERVALL = 0.05
...
class RPiZA:
    ...
    def _warte_fga_gruen(self, max_sekunden: float):
        ...
        phasen_dauer = max(max_sekunden / spd, T_MIN_GRUEN_FGA)
        start = time.monotonic()
    ...
    def _zyklus(self):
        while not self._stop_evt.is_set():
            ...
            if not self._warte_skaliert(self._fza_gruen_dauer()):
                break
...
# ============================================================
# GUI  (nur verfügbar, wenn tkinter installiert ist)
# ============================================================
...
if _HAS_TK:
    ...
    class RPiZAGui:
        def __init__(self, root, steuerung: RPiZA, mock: bool):
            self.root = root
        ...
        def _timer_text(self) -> str:
            ...
            if zustand == RPiZA.S_FZA_GRUEN:
                ...
                gesamt = max(s.fga_gruen_zeit / spd, T_MIN_GRUEN_FGA)
        ...
        def _aktualisiere(self):
            ...
            if self.mock:
                ...
                self._sensor_lbl.config(
                    text="Fahrzeug erkannt!" if erkannt else "Kein Fahrzeug",
...
def main():
    if not _HAS_TK:
        raise RuntimeError("tkinter nicht verfügbar — GUI kann nicht gestartet werden.")
